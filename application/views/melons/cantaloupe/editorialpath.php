...
<?php
	$this->template->add_css('application/views/widgets/ckeditor/custom.css');
	...
	$this->template->add_js('application/views/widgets/ckeditor/ckeditor.js');

	$this->template->add_js('if ( window.CKEDITOR && ( !CKEDITOR.env.ie || CKEDITOR.env.version > 7 ) ){CKEDITOR.env.isCompatible = true;}','embed');

	$this->template->add_js('application/views/widgets/edit/jquery-ui-custom/jquery-ui.min.js');
	$this->template->add_js('application/views/widgets/edit/jquery.ui.touch-punch.min.js');
	...
	$this->template->add_js('application/views/melons/cantaloupe/js/bootbox.min.js');
	$this->template->add_js('application/views/melons/cantaloupe/js/fuse.min.js');
	$this->template->add_js('application/views/widgets/edit/jquery.predefined.js');
	...
	$this->template->add_js('application/views/arbors/html5_RDFa/js/jquery.rdfquery.rules-1.0.js');
	$this->template->add_js('application/views/arbors/html5_RDFa/js/jquery.RDFa.js');
	$this->template->add_js('application/views/arbors/html5_RDFa/js/form-validation.js?v=2');
	$this->template->add_js('application/views/widgets/nav/jquery.scalarrecent.js');
	$this->template->add_js('application/views/widgets/cookie/jquery.cookie.js');
	$this->template->add_js('application/views/widgets/api/scalarapi.js');

	$this->template->add_js('application/views/widgets/diff/diff_match_patch.js');
	$this->template->add_js('application/views/widgets/diff/scalar_diff.js');
	$this->template->add_css('application/views/widgets/diff/scalar_diff.css');

	$this->template->add_js('application/views/melons/cantaloupe/js/tinysort.min.js');
	$this->template->add_css('application/views/melons/cantaloupe/css/editorial_path.css');
	$this->template->add_js('application/views/melons/cantaloupe/js/scalareditorialpath.jquery.js');
	$this->template->add_js('var views='.json_encode($this->config->item('views')), 'embed');
	$this->template->add_css('application/views/widgets/ckeditor/custom.css');
	$this->template->add_css('application/views/widgets/ckeditor/plugins/scalar/styles/scalar.css');
				...
				<div id="pathOrderSelectionContainer">
					...
					<div class="dropdown" id="contentOrderDropdown">
					  ...
					  <ul class="dropdown-menu dropdown-menu-left" aria-labelledby="contentOrderDropdownButton">
					    ...
					    <li><a href="#" data-sort="EditorialStateAsc">Editorial state ascending</a></li>
					    <li><a href="#" data-sort="EditorialStateDesc">Editorial state descending</a></li>
					  </ul>
					</div>
				</div>
			</heading>
