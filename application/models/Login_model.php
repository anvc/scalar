...
class Login_model extends User_model {
    ...
    public function get() {
		...
		//$data = $_SESSION[$this->login_basename];
		log_message('debug', "Session Data for login_basename " . $this->login_basename . ": " . print_r($data, TRUE));

		if (isset($_SESSION[$this->login_basename])) {
			log_message('debug', "Session Data from _SESSION: " . print_r($_SESSION[$this->login_basename], TRUE));
		}
		else {
			log_message('debug', "No data for $this->login_basename in _SESSION variable");
		}
	...
	public function do_login($force=false) {
		...
		if ($force || $action == 'do_login') {
			...
			if (!$result) {
				...
				parent::save_reset_string($result->user_id, '');  // Get rid of any reset string if the user successfully logs in
				$result->is_logged_in = true;
				log_message('debug', "User data recsult: " . print_r($result, TRUE));
				$this->session->set_userdata(array($this->login_basename => (array) $result));
				$this->reset_login_attempts();
