...
class User_model extends MY_Model {
    ...
    public function get_all($book_id = 0,
							$is_live = false,
							$orderby = 'fullname',
							$orderdir = 'asc',
							$total = null,
    ...
    public function get_by_user_id($user_id='') {
		log_message('debug', "get_by_user_id, user_id = $user_id");
		if ($query = $this->db->get_where($this->users_table, array('user_id' => $user_id))) {
			$result = $query->result();
			if (count($result) > 0) {
				return($result[0]);
			}
			else {
				log_message('error', "Could not retrieve user info for id: $user_id");
			}
		}
		return FALSE;
    }
    ...
    public function get_by_email_and_password($email='', $password='') {
    	...
    	$query = $this->db->get_where($this->users_table,
			array('email' => $email, 'password' => $this->get_hash($password)));
    	if ($query->num_rows() == 0) return false;
    	$result = $query->result();
    	return $result[0];
    }
    ...
    public function get_by_email_and_reset_string($email='', $reset_string='') {
		...
		$query = $this->db->get_where($this->users_table,
			array('email' => $email, 'reset_string' => $reset_string));
    	if ($query->num_rows() == 0) return false;
    	$result = $query->result();
    	return $result[0];
    ...
    public function get_by_fullname($fullname='') {
		$query = $this->db->get_where($this->users_table, array('fullname' => $fullname));
    	if ($query->num_rows() == 0) return false;
    ...
    public function get_super_admins() {
    	...
    	$query = $this->db->get_where($this->users_table, array('is_super' => 1));
    	if ($query->num_rows() == 0) return false;
    	return $query->result();  	// Could be more than one
    ...
    public function add($array=array()) {
		...
		if ($this->db->insert($this->users_table, $data)) {
			...
			if (!$user_id > 0) {
				if ($query = $this->db->query('SELECT LAST_INSERT_ID()')) {
					$row = $query->row();
					if (isset($row)) {
			...
			if ($user_id > 0) {
				log_message('info', "Added user, id: $user_id, fallback = $fallback");
			}
			else {
				log_message('error', "Unable to get last record id for added user");
			}
		}
		else
		{
			$inserterr = $this->db->error();
			if (!empty($inserterr)) {
				echo 'Database error: ' . $inserterr['message'] . ", Code: " . $inserterr['code'];
				log_message('error', "failed to add new user: " . $inserterr['message'] . ", Code: " . $inserterr['code']);
			}
		}
    ...
    public function register($array=array()) {
		...
		// 2023-0226 BH, recaptcha lib is now installed via composer and should be automatically auto loaded
