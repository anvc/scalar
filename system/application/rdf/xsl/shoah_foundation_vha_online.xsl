...
<xsl:stylesheet version="1.0"
    ...
    <!-- Shoah Foundation VHA Online "XML for Scalar" import XSL version 1.1 by Craig Dietrich -->                        
  	...
  	<!-- Variales -->
   	...
   	<xsl:variable name="archiveName">
		<xsl:text>Shoah Foundation Institute Visual History Archive Online</xsl:text>
	</xsl:variable>         
	...
	<xsl:template match="/shoah:Project4ScalarClass//shoah:TestimonyClass">	
        ...
        <xsl:choose>
          <xsl:when test="shoah:Tapes/shoah:TapeClass">
	...
	<xsl:template match="*"></xsl:template>
  	...
  	<xsl:template match="shoah:Tapes/shoah:TapeClass">
  		...
  		<rdf:Description rdf:about="{$VHAURL}&amp;TapeNumber={shoah:TapeNumber}">
  			...
  			<dcterms:description><xsl:value-of select="$TestimonyName"/><xsl:text>, </xsl:text><xsl:value-of select="$Experience"/><xsl:text>, Tape </xsl:text><xsl:value-of select="shoah:TapeNumber"/><xsl:text> (VHA Online)</xsl:text></dcterms:description>
	...
	<xsl:template name="noURL">
  		...
  		<xsl:param name="OutTapeNumber" />
  		<xsl:param name="OutTimecode" />
  		<rdf:Description rdf:about="VHA-{$IntCode}-noURL">
  			<dcterms:title><xsl:value-of select="$TestimonyName"/></dcterms:title> 	
  			<dcterms:description><xsl:value-of select="$TestimonyName"/><xsl:text>, </xsl:text><xsl:value-of select="$Experience"/><xsl:text>, data only</xsl:text></dcterms:description>
  			<dcterms:source><xsl:value-of select="$archiveName"/></dcterms:source>
  			<dcterms:identifier><xsl:value-of select="$IntCode"/></dcterms:identifier>
  			<dcterms:language><xsl:value-of select="$Language"/></dcterms:language>
  			<dcterms:subject><xsl:value-of select="normalize-space($Experience)"/></dcterms:subject>
