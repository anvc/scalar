...
.dialog {line-height:150%; margin-top:6px; margin-bottom:6px;}
...
function resizeList() {
	$('.search_results_wrapper').height(Math.max(200, $(window).height() - ($('.search_results_wrapper').offset().top + 210))+'px'); // magic number to get list height right
}
...
function search_archive(pagenum) {
	...
	// Enter the query and page number into the URI
...
function import_parse_xml_with_jquery(xml, sq, pagenum, paginate, uri, keep_hash) {
		...
		// Set up header
		$('.search_archive_form_wrapper:first').find('.search_archive_results tbody').empty();
		$('.search_results_header').empty();
		var $found_row = $('Found <b class="import_num_results">'+num_results+'</b> results (<b class="import_num_valid">'+num_rows+'</b> supported) for "'+sq+'"&nbsp; <span class="pagenum"></span>').appendTo('.search_results_header');
		...
		$results.each(function(indexInArray) {
			...
			if ('Internet Archive' == source) {
				...
				if (ia_pass) {
					...
					$filename.append('This media type is not presently supported by Scalar ['+basename(resource)+']');
					$tr.addClass('no_filename');
				}
			...
			// All others
				...
				if (filename.length) {
					...
					$filename.append("This media does not appear to have a file URL; import will not include the media content");
					$tr.addClass('no_filename');
				}
			...
			$('.search_archive_form_wrapper:first .search_archive_results tbody').append($tr);
			$('.search_archive_form_wrapper:first .import_num_results').html(num_rows);
			$('.search_archive_form_wrapper:first .import_num_valid').html(num_results);
			
		});
		
		// Redraw header
		$('.search_archive_form_wrapper:first .search_results_title').hide();	
		$('.search_archive_form_wrapper:first').find('.found_row_2').remove();
		var $found_row_2 = $('<div class="found_row_2" style="float:left;">Found <b id="import_num_results">'+num_results+'</b> supported results &nbsp; <span class="checkall"><input type="checkbox" name="checkall" id="checkall" /><label for="checkall"> Check all</label></span> &nbsp;	<span class="checkall"><input type="checkbox" name="checkallvalid" id="checkallvalid" /><label for="checkallvalid"> All with valid URLs</label></span> &nbsp; <span class="pagenum"></span></div>').prependTo('.search_results_footer');
		$found_row_2.find("input[name='checkall']").click(function() {
			$found_row_2.find("input[name='checkallvalid']").attr('checked', false);
			$('.search_archive_results').find("input[type='checkbox']").attr('checked', false);
			var is_checked = $(this).is(':checked');
			$('.search_archive_results').find("input[type='checkbox']").attr('checked', is_checked);
		});
		$found_row_2.find("input[name='checkallvalid']").click(function() {
			...
			$('.search_archive_results').find("input[type='checkbox']").attr('checked', false);
			if (is_checked) {
				$('.search_archive_results').find("input[type='checkbox']").each(function() {
					if ($(this).closest('tr').hasClass('no_filename')) return;
...
function import_parse_xml_with_javascript(xml, sq, pagenum, paginate, uri, keep_hash) {
		...
		// Set up header
		$('.search_archive_form_wrapper:first .search_archive_results tbody').empty();
		$('.search_results_header').empty();
		var $found_row = $('Found <b class="import_num_results">'+num_results+'</b> results (<b class="import_num_valid">'+num_rows+'</b> supported) for "'+sq+'"&nbsp; <span class="pagenum"></span>').appendTo('.search_results_header');
		...
		for (var j = 0; j < results.length; j++) {
			...
			$tr.data('node_uri', node_uri);
			$tr.data('fields', import_remap_fields(the_node, keep_hash));
			$('.search_archive_form_wrapper:visible .search_archive_results tbody').append($tr);
			$('.search_archive_form_wrapper:visible .import_num_results').html(num_rows);
			$('.search_archive_form_wrapper:visible .import_num_valid').html(num_results);
			
		}
		
		// Redraw header
		$('.search_archive_form_wrapper:visible .search_results_title').hide();		
		$('.search_archive_form_wrapper:visible').find('.found_row_2').remove()
...
function do_save_archive_import(nodes, saved_nodes) {  // I think technically multiple ajax calls can be made at same time, but just in case, run as a queue
	...
	if (saved_nodes.length == nodes.length) {
		...
		$box.append(((saved_nodes.length>1)?'Files have been':'A file has been')+' imported.  You may follow the link'+((saved_nodes.length>1)?'s':'')+' below to access the imported media. Alternatively, you can use the media section of the navigation (on the left) or the \'Manage content\' area (above) to browse all media by title.');
		$('body').append($box);
		// Add links
	...
	var error = function(err) {
		alert('Something went wrong while attempting to save: '+err);
	}
...
To begin, enter a search keyword in the input box below.

<br /><br />

<div class="search_archive_form_wrapper" id="external">	
	<?=$external->getPropValue('dcterms:description')?>
	<br /><br />	
...
<? endforeach ?>
	...
	<p class="search_results_title"><img src="<?=base_url()?>/system/application/views/modules/chrome/honeydew/images/loading.gif" height="16" align="absmiddle" />&nbsp; Searching the archive (may take a moment)</p>
