...
<?$this->template->add_css(path_from_file(__FILE__).'tablesorter/style.css')?>
		...
		function tagsGetContainerOf(the_link) {
			...
			if (!$the_link.data('is_open')) {
				...
				var data = {action:'get_tag_of',version_id:version_id}
				$.get('api/get_tag_of', data, function(data) {
						if (data.length == 0) {
							$the_row.after('<tr class="container_of_wrapper" id="tag_of_row_'+version_id+'"><td>&nbsp;</td><td class="odd" colspan="8">No items</td></tr>');
						} else {
						   	var $row = $('<tr class="container_of_wrapper" id="tag_of_row_'+version_id+'"><td colspan="8" style="padding:0px 0px 0px 0px;"><table style="width:100%;" cellspacing="0" cellpadding="0"></table></td></tr>');
						   	var $header = ('<tr><th></th><th style=\"display:none;\">ID</th><th>URI</th><th>Title</th><th>Description</th></tr>');
						   	$row.find('table').html($header);
						   	$the_row.after($row);			
						    for (var j in data) {
						    	var $data_row = $('<tr class="bottom_border tag_of_row"></tr>');
		...
		<table cellspacing="0" cellpadding="0" class="tablesorter">
			...
			<tbody>
<?
		if (!empty($current_book_tags)) {
			foreach ($current_book_tags as $row) {		
				$slug = $row->slug;
				...
				echo '<tr typeof="pages" id="tag_row_'.$row->content_id.'" class="bottom_border version content_id_'.$row->content_id.' version_id_'.$version->version_id.' '.(($row->is_live)?'':'not_live').'">';
				...
				echo '<td style="white-space:nowrap;text-align:center;"><a href="javascript:;" onclick="tagsGetContainerOf(this)" class="generic_button">View</a></td>';
				echo "</tr>\n";													
			}
		}
?>
		...
		&nbsp; <small>Caution: will delete checked tag relationship <b>and the tag page</b></small>
		</form>			
