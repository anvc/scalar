...
?>	
		...
		function repliesGetContainerOf(the_link) {
			...
			if (!$the_link.data('is_open')) {
				...
				$.get('api/get_reply_of', data, function(data) {
					$('#reply_of_row_'+version_id).remove();
					if (data.length == 0) {
						$the_row.after('<tr class="container_of_wrapper" id="reply_of_row_'+version_id+'"><td>&nbsp;</td><td class="odd" colspan="8">No items</td></tr>');
					} else {
						var $row = $('<tr class="container_of_wrapper" id="reply_of_row_'+version_id+'"><td colspan="8" style="padding:0px 0px 0px 0px;"><table style="width:100%;" cellspacing="0" cellpadding="0"></table></td></tr>');
						var $header = ('<tr><th></th><th style=\"display:none;\">ID</th><th>URI</th><th>Title</th><th>Description</th><th>Date/time</th></tr>');
						$row.find('table').html($header);
						$the_row.after($row);			
						for (var j in data) {
							var $data_row = $('<tr class="bottom_border reply_of_row"></tr>');
							...
							$data_row.append('<td property="start_seconds">'+data[j]['rel_created']+'</td>');
		...
		<table cellspacing="0" cellpadding="0" class="tablesorter">
			<thead>
				<tr class="head">
					...
					<th>Content</th>
					<th>Created</th>
					<th style="white-space:nowrap;">Comments on</th>
				</tr>
			</thead>
			<tbody>
<?
		if (!empty($current_book_replies)) {
			foreach ($current_book_replies as $row) {		
				$slug = $row->slug;
				...
				echo '<tr typeof="pages" id="reply_row_'.$row->content_id.'" class="bottom_border version content_id_'.$row->content_id.' version_id_'.$version->version_id.' '.(($row->is_live)?'':'not_live').'">';
				...
				echo '<td style="white-space:nowrap;text-align:center;"><a href="javascript:;" onclick="repliesGetContainerOf(this)" class="generic_button">View</a></td>';
				echo "</tr>\n";													
			}
		}
?>
