...
class RDF_Object {
		...
		for ($j = 0; $j < count($row->versions); $j++) {
			...
			foreach ($models as $model) {
				...
				if ($settings['rel'] == self::REL_PARENTS_ONLY || $settings['rel'] == self::REL_ALL) {
					...
					foreach ($nodes as $node) {
						$parent = $this->_annotation_parent($node, $settings);
						if (null!==$parent) array_push($row->versions[$j]->$name, $parent);
					}
				}
				if ($settings['rel'] == self::REL_CHILDREN_ONLY || $settings['rel'] == self::REL_ALL) {
					...
					foreach ($nodes as $node) {
						$child = $this->_annotation_child($node, $settings);
	...
	protected function _annotation_parent($annotation, $settings) {
		...
		} elseif (!empty($settings['use_versions']) && !array_key_exists($annotation->parent_content_id, $settings['use_versions'])) {
			return null;
		} elseif ($settings['use_versions_restriction'] >= self::USE_VERSIONS_EXCLUSIVE && !empty($settings['use_versions']) && $settings['use_versions'][$annotation->parent_content_id] != $annotation->parent_version_id) {
			return null;
		}
		++$settings['num_recurses'];
	...
	protected function _annotation_child($annotation, $settings) {
		...
		} elseif ($settings['use_versions_restriction'] >= self::USE_VERSIONS_EXCLUSIVE && !empty($settings['use_versions']) && $settings['use_versions'][$annotation->child_content_id] != $annotation->child_version_id) {
			return null;
		}
		++$settings['num_recurses'];
		$content = $this->_content($content, $settings);
