...
To install Scalar on your own server, you can download the most recent build from GitHub. Or, if you are concerned about downloading from the "live" GitHub codebase, we preiodically ZIP Scalar into a "version" file and make available at: https://github.com/anvc/scalar/wiki/Scalar-versions-for-download . The code kept in the ZIP files can be assumed to be tested in both development and live environments (e.g., scalar.usc.edu).  For help installing Scalar, see either INSTALL.txt or UPDATE.txt in the project root folder.
